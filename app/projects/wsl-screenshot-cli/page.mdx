...
A persistent `powershell.exe -STA` subprocess handles all clipboard access via a custom stdin/stdout text protocol (`CHECK` / `UPDATE` / `EXIT`). It originally used a runtime-compiled C# `ClipboardListener` class (via `Add-Type -TypeDefinition`) with `AddClipboardFormatListener` for event-driven clipboard detection. A [community contribution](https://github.com/Nailuu/wsl-screenshot-cli/pull/4) switched to pre-compiled .NET Clipboard APIs (`ContainsImage()`, `ContainsText()`, `ContainsFileDropList()`) because enterprise EDR software (CrowdStrike, SentinelOne, etc.) blocks `csc.exe`, which `Add-Type` requires for runtime C# compilation. The pre-compiled APIs provide the same detection without needing `csc.exe`. PowerShell pumps Windows messages via `DoEvents()` to keep the STA thread responsive. When a new bitmap is detected, it saves the PNG (deduplicated by SHA256 hash) and sets three clipboard formats at once:
...
The `--quiet` flag suppresses informational messages when the daemon is already running, keeping your terminal clean on every new shell session.
...
Other tools that solve this problem monitor the Windows screenshot auto-save folder, but there's no official Windows API to reliably get that folder path, making them fragile. My approach: instead of watching files, intercept at the clipboard level using pre-compiled .NET Clipboard APIs (`ContainsImage()`, `ContainsText()`, `ContainsFileDropList()`).
...
The clipboard detection initially used a runtime-compiled C# class (`Add-Type -TypeDefinition`) for event-driven clipboard listening, but a [community contribution](https://github.com/Nailuu/wsl-screenshot-cli/pull/4) switched to fully pre-compiled .NET Clipboard APIs for enterprise EDR compatibility — tools like CrowdStrike and SentinelOne block `csc.exe`, which `Add-Type` requires.
...
The tool has benefited from open-source contributions that improved real-world compatibility:

- **EDR compatibility** — Switched from runtime-compiled C# (`Add-Type`) to pre-compiled .NET Clipboard APIs, fixing silent failures in enterprise environments where EDR software blocks `csc.exe` ([PR #4](https://github.com/Nailuu/wsl-screenshot-cli/pull/4))
- **`--quiet` flag** — Suppresses informational messages when the daemon is already running, enabling clean `.bashrc`/`.zshrc` autostart ([PR #2](https://github.com/Nailuu/wsl-screenshot-cli/pull/2))
- **Broader WSL detection** — Added `/proc/version` fallback for WSL environment detection when `wslinfo` flags aren't available ([PR #1](https://github.com/Nailuu/wsl-screenshot-cli/pull/1))
