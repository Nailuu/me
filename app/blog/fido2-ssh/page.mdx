...
SSH will try each key in config file order. Plug in the corresponding YubiKey and you will be prompted for a touch. On success:
...
Without the physical YubiKey (and your PIN), authentication is impossible. These stub files are safe to back up since they are useless without the corresponding YubiKey.
